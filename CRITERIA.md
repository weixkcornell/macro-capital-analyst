...
| `version-lockstep` | hard | pack.json 与 10 个维度的 version 字段 | 全部实体/配置文件的 version 必须与 pack.json 一致 | README/清单里的版本（那是 doc-version 的活）；脚本自身的"版本" | `version-drift`（有对照）、`missing-version`（有对照） |
| `doc-version` | hard | README 徽章 / README 版本历史首条 / SUBMISSION-CHECKLIST 的 version= | 三处必须等于 pack.json 的 version；版本历史首条不得是占位（bump-version 生成） | README 正文里出现的其他版本字样（历史条目本就该保留旧版本号） | `doc-version-drift`（有对照）、`missing-version-badge`（有对照）、`missing-version-history`（有对照）、`missing-readme`（有对照）、`missing-checklist`（有对照）、`missing-checklist-version`（有对照）、`release-note-placeholder`（有对照） |
...
| `negative-controls` | hard | 判据 × 负向对照矩阵（门禁自己的覆盖率） | 自有 code 必须 100% 有负向对照（在 selftest-gates.mjs 里能被注入式缺陷触发） | 平台校验器自带的 16 个 code（归平台，显式白名单排除）；"对照是否恰好打在该 code 的判据上" | `negative-control-gap`（有对照） |
| `criteria-doc` | hard | CRITERIA.md 与上面的 CRITERIA 登记表 | CRITERIA.md 必须存在，且其 ```json 代码块与登记表逐字一致 | 文档措辞的可读性；登记表"是否覆盖了所有该有的判据"（需要人工审阅） | `criteria-doc-missing`（有对照）、`criteria-doc-drift`（有对照） |
